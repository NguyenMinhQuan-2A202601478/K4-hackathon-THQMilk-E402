...
def find_data_discord_path() -> Path:
    """Find data/data-discord path across candidate locations."""
    candidates = [
        Path("data/data-discord"),
        Path("../data/data-discord"),
        Path(__file__).resolve().parent.parent / "data" / "data-discord",
...
def is_noise_message(text: str) -> bool:
    """Filter out short greetings, casual chatter, or non-announcement messages."""
...
def is_candidate_announcement(msg: dict[str, Any]) -> bool:
    """Identify candidate announcements based on author roles or key keywords."""
...
def load_and_group_messages(custom_dir: Path | None = None) -> dict[str, Any]:
    """Scan and parse JSON files from data/data-discord."""
